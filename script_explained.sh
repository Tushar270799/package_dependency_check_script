...
# RECURSIVE: controls whether to do deps-of-deps (full tree) or just direct deps
# 0 = off (default) = only show what the target DIRECTLY needs
# 1 = on  (set by -r flag) = also show what those packages need, and so on
RECURSIVE=0
...
#   OPTIONS:     start with - or -- like: -r, -v, --recursive, --help
...
while [[ $# -gt 0 ]]; do
    ...
    case "$1" in
        ...
        # -r or --recursive: user wants full transitive dependency tree
        # Set RECURSIVE flag to 1 (on)
...
#   - Once for the direct dependencies (Section 14)
#   - Repeatedly in the BFS loop for recursive/transitive deps (Section 15)
# =============================================================================
...
# =============================================================================
# SECTION 16 — OPTIONAL RECURSIVE PASS (BFS)
# =============================================================================
# If user passed -r flag, also find deps-of-deps-of-deps... (full tree).
#
# Uses Breadth-First Search (BFS) algorithm:
#   - Start with direct deps in a queue
#   - Process each package in queue: find ITS deps, add new ones to queue
#   - Keep going until queue is empty (no new packages found)
#   - visited set prevents infinite loops from circular dependencies
#
# Example: htop → glibc → (glibc's deps: nothing new) → done
#          firefox → libssl → libcrypto → glibc → (already seen) → done
# =============================================================================

# Check if recursive mode is on AND we have at least one dep to recurse into
# $RECURSIVE -eq 1     = -r flag was passed
# &&                   = AND
# ${#FOUND_PKGS[@]} -gt 0  = FOUND_PKGS has at least one entry
#   ${#FOUND_PKGS[@]} = count of keys in FOUND_PKGS
#   -gt 0             = greater than zero
if [[ $RECURSIVE -eq 1 && ${#FOUND_PKGS[@]} -gt 0 ]]; then
    info "Resolving transitive dependencies…"

    # ALL_PKGS = accumulator: collects EVERY package seen across ALL BFS iterations
    # Starts with the direct deps, grows as we discover more
    declare -A ALL_PKGS=()

    # Seed ALL_PKGS with the direct dependencies already found
    # for p in "${!FOUND_PKGS[@]}"  = loop over all KEYS of FOUND_PKGS
    # ALL_PKGS["$p"]=1              = add each key to ALL_PKGS
    for p in "${!FOUND_PKGS[@]}"; do ALL_PKGS["$p"]=1; done

    # bfs_queue = the processing queue for BFS
    # declare -a = indexed array (used as FIFO queue)
    # ("${!FOUND_PKGS[@]}")  = initialize with all direct dep package names
    declare -a bfs_queue=("${!FOUND_PKGS[@]}")

    # visited = set of packages we've already fully processed
    # Prevents processing the same package twice
    # Critical for correctness when packages have circular dependencies
    # (A needs B, B needs A — without visited, this would loop forever)
    declare -A visited=()

    # BFS main loop: keep processing until the queue is empty
    while [[ ${#bfs_queue[@]} -gt 0 ]]; do

        # DEQUEUE: take the FIRST element from the front of the queue
        # bfs_queue[0]  = element at index 0 (front of queue)
        current="${bfs_queue[0]}"

        # Remove the first element by slicing the array from index 1 onward
        # "${bfs_queue[@]:1}"  = array slice: all elements starting from index 1
        # :1                   = start at index 1, go to end
        # This effectively removes index 0, making old index 1 the new index 0
        # Bash has no built-in queue, so we simulate one this way
        bfs_queue=("${bfs_queue[@]:1}")

        # Check if we've already processed this package — skip if so
        # ${visited[$current]+_}  = exists test: if key exists, returns "_" (truthy)
        # && continue             = if already visited, skip to next queue item
        [[ "${visited[$current]+_}" ]] && continue

        # Mark this package as visited so we don't process it again
        visited["$current"]=1

        # Get all ELF files from this dependency package
        # 2>/dev/null = suppress warnings from elf_files_from_installed
        # || true     = if package not found, produce empty output (don't die)
        mapfile -t sub_elfs < <(elf_files_from_installed "$current" 2>/dev/null || true)

        # If no ELF files found for this package, nothing to analyse — skip
        # ${#sub_elfs[@]} -eq 0  = array is empty
        [[ ${#sub_elfs[@]} -eq 0 ]] && continue

        # Run dependency analysis on this package's ELF files
        # Results are added to global FOUND_PKGS map
        collect_deps_for_elfs < <(printf '%s\n' "${sub_elfs[@]}")

        # Check for newly discovered packages and add them to the queue
        for p in "${!FOUND_PKGS[@]}"; do
            # ${ALL_PKGS[$p]+_}  = has this package already been seen before?
            # || bfs_queue+=("$p")  = if NOT already seen, add to queue
            # +=("$p")  = append element to end of array
            [[ "${ALL_PKGS[$p]+_}" ]] || bfs_queue+=("$p")

            # Mark as seen in ALL_PKGS regardless
            ALL_PKGS["$p"]=1
        done
    done

    # After BFS completes, merge ALL_PKGS back into FOUND_PKGS
    # ALL_PKGS has the complete set of all packages found (direct + transitive)
    for p in "${!ALL_PKGS[@]}"; do FOUND_PKGS["$p"]=1; done
fi


# =============================================================================
# SECTION 17 — OUTPUT: REQUIRED PACKAGES LIST
# =============================================================================
...
# =============================================================================
# SECTION 18 — OUTPUT: UNRESOLVED LIBRARIES (only shown with -u flag)
# =============================================================================
